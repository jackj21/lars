...
- $\begin{cases}r > 0 & ;\ \text{increase } \beta _{j}\\r < 0 & ;\ \text{decrease } \beta _{j}\end{cases}$, 
and calculate residuals of the new model fit ($\hat{y}=\beta_jx;\ r=y-\hat{y}$)
...
<center>$< r,x_j>=0\forall j$</center>
...
```{r, fig.width=7.5, fig.height=5.75}
plot(lars_fit)
```
