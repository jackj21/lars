...
```{r echo = TRUE, warning = FALSE, error = FALSE, message = FALSE}
fit1 = predict.lars(us.lars,X, s = 1)
fit2 = predict.lars(us.lars,X,s=2)


ggplot(USArrests, aes(x=fit2$fit, y=y)) + 
...
car.x <- as.matrix(subset(mtcars, select=-c(mpg)))
head(car.x, 5)
```
...
car.lars <- lars(car.x, mtcars$mpg, type="lar", trace=TRUE, 
